...
class EntrepriseFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        ent = Entreprise(
            nom=nom, secteur_activite=secteur_activite, adresse=adresse, siret=siret, owner_id=owner_id, **kwargs
        )
        db.add(ent)
        db.commit()
        db.refresh(ent)
...
class ContactFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        contact = Contact(entreprise_id=entreprise_id, nom=nom, role=role, email=email, telephone=telephone, **kwargs)
        db.add(contact)
        db.commit()
        db.refresh(contact)
        return contact
...
class SiteFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        site = Site(nom=nom, entreprise_id=entreprise_id, adresse=adresse, description=description, **kwargs)
        db.add(site)
        db.commit()
        db.refresh(site)
        return site
...
class AuditFactory:
    ...
    @staticmethod
    def create(
        db: Session, nom_projet: str = "Test Audit", entreprise_id: int = None, status: str = "NOUVEAU", **kwargs
    ) -> Audit:
        """Create an audit with defaults"""
        from app.models.audit import AuditStatus

        audit = Audit(nom_projet=nom_projet, entreprise_id=entreprise_id, status=AuditStatus(status), **kwargs)
        db.add(audit)
        db.commit()
        db.refresh(audit)
        return audit
...
class FrameworkFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        fw = Framework(ref_id=ref_id, name=name, description=description, version=version, **kwargs)
        db.add(fw)
        db.commit()
        db.refresh(fw)
        return fw
...
class FrameworkCategoryFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        cat = FrameworkCategory(framework_id=framework_id, name=name, description=description, order=order, **kwargs)
        db.add(cat)
        db.commit()
        db.refresh(cat)
        return cat
...
class ControlFactory:
    ...
    @staticmethod
    def create_batch(db: Session, category_id: int, count: int = 5) -> list[Control]:
...
class AssessmentCampaignFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        campaign = AssessmentCampaign(audit_id=audit_id, name=name, description=description, status=status, **kwargs)
        db.add(campaign)
        db.commit()
        db.refresh(campaign)
        return campaign
...
class AssessmentFactory:
    ...
    @staticmethod
    def create(
        db: Session, campaign_id: int, equipement_id: int, framework_id: int, notes: str = "", **kwargs
    ) -> Assessment:
        """Create an assessment"""
        assessment = Assessment(
            campaign_id=campaign_id, equipement_id=equipement_id, framework_id=framework_id, notes=notes, **kwargs
        )
        db.add(assessment)
        db.commit()
...
def create_full_assessment_scenario(db: Session):
    ...
    auditeur = UserFactory.create(
        db, username=f"auditeur_{unique_id}", email=f"auditeur_{unique_id}@test.local", role="auditeur"
    )
