...
class TestPathConstruction:
    ...
    def test_path_structure_single_tenant_id(self):
        ...
        assert slug_part < cloud_idx < m365_idx < scan_idx, (
            f"Path parts out of order: slug({slug_part}) < Cloud({cloud_idx}) < M365({m365_idx}) < scan_id({scan_idx})"
        )
    ...
    def test_cloud_m365_structure_consistency(self):
        ...
        for company in ["Company A", "Company B", "Test Org"]:
            for scan_id in ["scan-1", "scan-2"]:
                ...
                assert path_str.endswith(scan_id), f"scan_id '{scan_id}' should be last in path: {path_str}"
...
class TestBackwardCompatibility:
    ...
    def test_old_audit_data_with_duplicate_paths_still_accessible(self, tmp_path):
        ...
        old_meta = {"scan_id": "scan_001", "status": "completed", "old_format": True}
    ...
    def test_new_path_structure_queries_don_not_break(self, tmp_path):
        ...
        with patch("app.core.storage.get_settings") as mock_settings:
            ...
            for i in range(3):
                ...
                # Write metadata
                meta = {"scan_id": scan_id, "status": "success", "findings_count": i * 10}
...
class TestEdgeCases:
    ...
    def test_special_characters_in_tenant_sanitized(self):
        ...
        for company, expected_slug in test_cases:
            ...
            assert slug == expected_slug, f"Slug mismatch for '{company}': got '{slug}', expected '{expected_slug}'"

            # Path should contain the slugified version
            assert slug in path_str or slug == "", f"Slug '{slug}' not found in path: {path_str}"
            ...
            for i, p1 in enumerate(paths):
                for j, p2 in enumerate(paths):
                    if i != j:
                        ...
                        assert not str(p2).startswith(str(p1)), f"Path nesting detected: {p1} contains {p2}"
                        assert not str(p1).startswith(str(p2)), f"Path nesting detected: {p2} contains {p1}"
    ...
    def test_findings_saved_in_correct_path(self, tmp_path):
        ...
        with patch("app.core.storage.get_settings") as mock_settings:
            ...
            findings = {
                "scan_id": scan_id,
                "findings_count": 42,
                "results": [{"issue": "Config X", "severity": "High"}, {"issue": "Config Y", "severity": "Medium"}],
            }
...
class TestIntegrationSmoke:
    ...
    def test_full_audit_lifecycle(self, tmp_path):
        ...
        with patch("app.core.storage.get_settings") as mock_settings:
            ...
            meta = {"scan_id": scan_id, "company": company, "findings_count": 15, "status": "completed"}
    ...
    def test_multiple_companies_isolation(self, tmp_path):
        ...
        with patch("app.core.storage.get_settings") as mock_settings:
            ...
            for company in companies:
                ...
                assert meta["company"] == company, f"Data contamination: {company} metadata contains {meta['company']}"
