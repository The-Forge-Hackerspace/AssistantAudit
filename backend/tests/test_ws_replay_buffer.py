...
class TestReplayBufferedEvents:
    ...
    def test_full_replay_clears_buffer(self, manager):
        ...
        user_id = 1
        manager.user_event_buffer[user_id] = [
            (_ts(), _make_event(i)) for i in range(5)
        ]

        ws = AsyncMock()
        asyncio.run(
            manager._replay_buffered_events(user_id, ws)
        )
