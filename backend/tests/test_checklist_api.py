...
class TestChecklistInstanceRoutes:
    ...
    def test_create_instance(self, client: TestClient, auditeur_headers, seeded_template, db_session, auditeur_user):
        ...
        resp = client.post(
            ...
            json={"template_id": seeded_template.id, "audit_id": audit.id},
    ...
    def test_respond_to_item(self, client: TestClient, auditeur_headers, seeded_template, db_session, auditeur_user):
        ...
        item = (
            ...
            .filter(ChecklistSection.template_id == seeded_template.id)
            .first()
        )
    ...
    def test_delete_instance_makes_it_non_retrievable(
        ...
        resp = client.delete(f"/api/v1/checklists/instances/{instance_id}", headers=auditeur_headers)
        assert resp.status_code == 200

        resp_get = client.get(f"/api/v1/checklists/instances/{instance_id}", headers=auditeur_headers)
    ...
    def test_complete_instance_sets_status_and_completed_at(
        ...
        resp = client.post(f"/api/v1/checklists/instances/{instance_id}/complete", headers=auditeur_headers)
        assert resp.status_code == 200
        data = resp.json()
