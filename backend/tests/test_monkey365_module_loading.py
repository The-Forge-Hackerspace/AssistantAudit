...
def test_ensure_monkey365_ready_module_exists(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout="Invoke-Monkey365\nOther commands...", stderr="")
...
def test_module_import_failure_raises_error(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=1, stdout="", stderr="Import-Module : The specified module 'monkey365.psm1' was not loaded"
        )
...
def test_module_import_command_not_found_raises_error(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout="Some-Other-Command\nGet-Help", stderr="")
...
def test_git_clone_failure_raises_error(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=128, stdout="", stderr="fatal: repository not found")
