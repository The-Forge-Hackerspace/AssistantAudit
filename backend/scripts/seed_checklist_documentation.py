...
def seed(db=None):
    ...
    if db is None:
        db = SessionLocal()
    try:
        existing = db.query(ChecklistTemplate).filter(ChecklistTemplate.name == CHECKLIST_DOCUMENTATION["name"]).first()

        if existing:
            print(f"Checklist '{CHECKLIST_DOCUMENTATION['name']}' déjà présente (id={existing.id}), rien à faire.")
            return
