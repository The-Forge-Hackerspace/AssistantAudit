...
class AuditReport(Base):
    ...
    # Statut du rapport
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="draft")  # draft, generating, ready, error
    # Template utilisé
    ...
    # Metadata
    generated_by: Mapped[int | None] = mapped_column(Integer, ForeignKey("users.id"), index=True)
    ...
    __table_args__ = (
        CheckConstraint("status IN ('draft', 'generating', 'ready', 'error')", name="ck_report_status"),
        CheckConstraint("template_name IN ('complete', 'light', 'compliance')", name="ck_report_template"),
    )
