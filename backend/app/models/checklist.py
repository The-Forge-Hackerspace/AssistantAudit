...
class ChecklistTemplate(Base):
    ...
    sections: Mapped[list["ChecklistSection"]] = relationship(
        back_populates="template", cascade="all, delete-orphan", order_by="ChecklistSection.order"
    )
...
class ChecklistSection(Base):
    ...
    items: Mapped[list["ChecklistItem"]] = relationship(
        back_populates="section", cascade="all, delete-orphan", order_by="ChecklistItem.order"
    )
...
class ChecklistInstance(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    template_id: Mapped[int] = mapped_column(Integer, ForeignKey("checklist_templates.id"), nullable=False, index=True)
    ...
    site_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("sites.id", ondelete="SET NULL"), index=True)
    filled_by: Mapped[int | None] = mapped_column(Integer, ForeignKey("users.id"), index=True)
    ...
    responses: Mapped[list["ChecklistResponse"]] = relationship(back_populates="instance", cascade="all, delete-orphan")
...
class ChecklistResponse(Base):
    ...
    item_id: Mapped[int] = mapped_column(Integer, ForeignKey("checklist_items.id"), nullable=False, index=True)
    # OK, NOK, NA (non applicable), UNCHECKED (non vérifié)
    status: Mapped[str] = mapped_column(String(10), nullable=False, default="UNCHECKED")
    note: Mapped[str | None] = mapped_column(Text)
    responded_by: Mapped[int | None] = mapped_column(Integer, ForeignKey("users.id"))
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('OK', 'NOK', 'NA', 'UNCHECKED')", name="ck_checklist_response_status"),
    )
