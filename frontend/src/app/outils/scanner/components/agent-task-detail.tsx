...
export function AgentTaskDetail({ task, open, onOpenChange, agentName }: AgentTaskDetailProps) {
  ...
  const hosts: NmapHost[] = (summary?.hosts as NmapHost[]) || [];
  ...
  const guessType = (host: NmapHost): TypeEquipement => {
    const ports = (host.ports || []).map((p) => p.port_number);
    ...
    for (const host of hosts) {
      ...
      try {
        await equipementsApi.create({
          ...
          os_detected: host.os_guess || undefined,
          ...
          <div>
            ...
            <Table>
              ...
              <TableBody>
                {hosts.map((host, i) => (
                  <TableRow key={i}>
                    <TableCell className="font-mono font-semibold">{host.ip_address || "—"}</TableCell>
                    <TableCell>{host.hostname || "—"}</TableCell>
                    <TableCell className="font-mono text-xs">{host.mac_address || "—"}</TableCell>
                    <TableCell>{host.vendor || "—"}</TableCell>
                    <TableCell className="text-sm">{host.os_guess || "—"}</TableCell>
                    <TableCell>
                      <div className="flex gap-1 flex-wrap">
                        {(host.ports || []).slice(0, 8).map((p, j) => (
                          <Badge key={j} variant="outline" className="text-[10px] font-mono">
                            {p.port_number}/{p.service_name || p.protocol || "?"}
                          </Badge>
                        ))}
                        {(host.ports?.length || 0) > 8 && (
